...
spline_ik_bones = ["c_spline_root", "c_spline_curvy"]

# OPERATOR CLASSES ###########################################################################################################    
class ARP_OT_reset_script(bpy.types.Operator):         
...
class ARP_OT_snap_pin(bpy.types.Operator):
...
class ARP_OT_arm_bake_fk_to_ik(bpy.types.Operator):
...
class ARP_OT_arm_bake_ik_to_fk(bpy.types.Operator):
...
class ARP_OT_leg_bake_fk_to_ik(bpy.types.Operator):
...
class ARP_OT_leg_bake_ik_to_fk(bpy.types.Operator):
...
def _set_picker_camera(self):    
    ...
    if cam_ui:
        ...
        if ui_mesh:            
            ...
            cam_ui.matrix_world = rig.matrix_world @ mathutils.Matrix.Translation((0, -40, picker_center[2]))
            cam_ui.scale = (1.0,1.0,1.0)
            cam_ui.rotation_euler = (math.radians(90), 0, 0)            
...
def snap_pos(pose_bone, target_bone):
    ...
    if pose_bone.parent:        
        ...
        #update hack
        bpy.ops.transform.translate(value=(0, 0, 0), constraint_axis=(False, False, False), orient_type='NORMAL', mirror=False)
        # second apply
        pose_bone.matrix = target_bone.matrix
    else:       
...
def snap_pos_matrix(pose_bone, target_bone_matrix):
    ...
    if pose_bone.parent:        
        # apply double time because of dependecy lag
        pose_bone.matrix = target_bone_matrix
        #update hack
        bpy.ops.transform.translate(value=(0, 0, 0), constraint_axis=(False, False, False), orient_type='NORMAL', mirror=False)
        # second apply
        pose_bone.matrix = target_bone_matrix
...
def fk_to_ik_leg(obj, side):   
    ...
    if c_thigh_b:
        print("restore mat")
        print(c_thigh_b.matrix)
        c_thigh_b.matrix = c_thigh_b_matrix
        print(c_thigh_b.matrix)
...
def ik_to_fk_leg(obj, side):    
    ...
    if c_thigh_b:
        c_thigh_b_matrix = c_thigh_b.matrix.copy()
        pole_fk_matrix = pole_fk.matrix.copy()
        """
        print("SAVED MATRIX")
        print(c_thigh_b_matrix)
        thigh_b_ik01 = get_pose_bone("thigh_b_ik01"+side)
        thigh_b_ik01_matrix = thigh_b_ik01.matrix.copy()
        """       
    ...
    # Snap Pole    
    snap_pos_matrix(pole, pole_fk_matrix)
    ...
    if c_thigh_b:
        print("restore matrix x")
        c_thigh_b.matrix = c_thigh_b_matrix
        """
        #c_thigh_b.matrix = thigh_b_ik01.matrix_channel.inverted() @ c_thigh_b_matrix
        c_thigh_b.matrix = mat_diff + c_thigh_b.matrix
        bpy.ops.transform.translate(value=(0, 0, 0))
        """  
...
class ARP_PT_rig_ui(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Get bone side
        bone_side = selected_bone_name[-2:]
        
        if "_dupli_" in selected_bone_name:
            bone_side = selected_bone_name[-12:]     
        
        def is_selected(names, startswith=False): 
            if startswith == False:
                if type(names) == list:             
                    for name in names:  
                        if not "." in name[-2:]:                        
        ...
        if not 'humanoid' in bpy.context.active_object.name:
           ...
           #LEG                     
            if (is_selected(fk_leg) or is_selected(ik_leg)):                
               ...
               # Stretch length property
                ...
                if bpy.context.scene.show_ik_fk_advanced:
                    ...
                    p.side = bone_side
                    p = col.operator("pose.arp_bake_leg_fk_to_ik", text="Bake FK > IK...")
                    p.side = bone_side
                    p = col.operator("pose.arp_bake_leg_ik_to_fk", text="Bake IK > FK...")
                    p.side = bone_side
            ...
            if is_selected(fk_arm) or is_selected(ik_arm):
               ...
               # Stretch length property
                ...
                if bpy.context.scene.show_ik_fk_advanced:
                    ...
                    p.side = bone_side
                    p = col.operator("pose.arp_bake_arm_fk_to_ik", text="Bake FK > IK...")
                    p.side = bone_side
                    p = col.operator("pose.arp_bake_arm_ik_to_fk", text="Bake IK > FK...")
                    p.side = bone_side
            ...
            if is_selected(fingers):
                ...
                if (bone_side[-2:] == ".r"):
                    finger_side = "Right "                
                text_upper = (selected_bone_name[:3]).upper()
                layout.label(text=finger_side + text_upper[2:] + selected_bone_name[3:-8] + ":")
                layout.prop(pose_bones[selected_bone_name], '["bend_all"]', text="Bend All Phalanges", slider=True)
            ...
            # SPLINE IK
            if is_selected("c_spline_", startswith=True):
                layout.label(text="Spline IK")
                spline_root = pose_bones["c_spline_root" + bone_side]
                ...
                layout.prop(pose_bones["c_spline_root" + bone_side], '["volume_variation"]', text = 'Volume Variation')
...
###########  REGISTER  ##################
classes = (ARP_OT_set_picker_camera_func, ARP_OT_toggle_multi, ARP_OT_arp_snap_pole, ARP_OT_arm_switch_snap, ARP_OT_arm_bake_fk_to_ik, ARP_OT_arm_fk_to_ik, ARP_OT_arm_bake_ik_to_fk, ARP_OT_arm_ik_to_fk, ARP_OT_leg_switch_snap, ARP_OT_leg_fk_to_ik, ARP_OT_leg_bake_fk_to_ik,  ARP_OT_leg_ik_to_fk, ARP_OT_leg_bake_ik_to_fk, ARP_PT_rig_ui, ARP_OT_snap_pin, ARP_OT_reset_script)
